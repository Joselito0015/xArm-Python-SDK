...
def on_message(ws, message):
    ...
    print(Parameters)
    if Parameters["motor"]=="stoped":
        print("Motor encendido")
        robot_main._vars = {'variable1': 2}      
    print(robot_main._vars)
            ...
            while self.is_alive:
                ...
                for i in range(int(1)):
                    if not self.is_alive:
                        break
                    # Recibir variable de la nube
                    # print("Conectado")
                    if self._vars.get('variable1', 0) == 2:
                        code = self._arm.set_position(x=10, radius=-1, speed=self._tcp_speed, mvacc=self._tcp_acc, relative=True, wait=True)
                        if not self._check_code(code, 'set_position'):
                            return
                        print("Cambió variable, el brazose movió 10 mm")
                    else:
                        pass
